...
void DetectPulse::Execute(Mat &image, Rect &face, bool faceFound)
{
	...
	buffer_fft.Execute(skinMean); //time for each frame is calculated
	...
	putText(image, text, Point(face.x-50, face.y +20), FONT_HERSHEY_PLAIN, size, Scalar(0, 255, 0));
	putText(image, text, Point(face.x - 50, face.y + 21), FONT_HERSHEY_PLAIN, size, Scalar(255, 0, 0));
